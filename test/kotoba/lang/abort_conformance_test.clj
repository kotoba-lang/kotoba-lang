(ns kotoba.lang.abort-conformance-test
  "Runs the abort ability's conformance cases (lang/abort-ability.edn, slice
  1) through the frontend and KIR, so `:conformance-vectors-positive-and-
...
(deftest the-manifest-declares-the-cases-the-contract-lists
  (let [contract (edn/read-string (slurp "lang/abort-ability.edn"))
    ...
    (is (= (set (concat (get-in contract [:conformance :positive])
           ...
           declared))
    (is (= 3 (count (filter #(= :run (:kind %)) (abort-cases)))))
    (is (<= 5 (count (filter #(= :expect-error (:kind %)) (abort-cases)))))))
