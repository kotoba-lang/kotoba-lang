...
(deftest the-module-provides-exactly-what-it-claims
  (let [contract (edn/read-string (slurp "lang/compat.edn"))
    ...
    (testing "and the three Clojure names stay absent, each with a reason"
      ;; The reasons live on the CANONICAL module since 2026-09-08, when
      ;; kotoba.string became the guest string library and clojure.string
      ;; became a shim over it. The shim's :absent map holds pointers, so
      ;; reading :instead there reads nothing -- which is exactly what this
      ;; assertion caught when the move landed without it.
      (let [absent (get-in contract [:modules :kotoba.string :absent])
            shim-absent (get-in contract [:modules :clojure.string :absent])]
        (doseq [name- '[lower-case upper-case capitalize]]
          ...
          (is (str/includes? (:reason (get shim-absent name-))
                             ":modules :kotoba.string :absent")
              (str name- " on the shim must point at the canonical reason")))))
