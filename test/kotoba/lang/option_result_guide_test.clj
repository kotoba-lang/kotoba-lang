...
(deftest guide-exists-and-covers-layers
  (let [g (slurp guide-path)]
    ...
    (is (str/includes? g "result-value")
        "the result half is the language builtin now, not the stdlib's unwrap-ok")
    (is (str/includes? g "withdrew the records")
        "the guide must say where the stdlib prelude layer went")
