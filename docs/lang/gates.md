...
The first command runs the manifest-driven conformance suite from
`lang/conformance/` through the implementation compiler crate. That suite covers
source-file `:run` cases,
`:compile-expr` inline-expression cases, and declared negative/error cases. The
second command pins the user-facing `kotoba -e` path. The third command runs
...
canonical `.kotoba` sources over `.clj` compatibility files. The fourth command
...
`-S` / `--source-path` handling. The fifth through seventh commands pin the
...
legacy `:clojure` / `:clj` aliases preserved. The eighth command pins the
...
`:provides` requirements, and `app.kotoba.*` contract surfaces in manifests and
lockfiles.
