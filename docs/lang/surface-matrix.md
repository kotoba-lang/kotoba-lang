...
| `explicit-errors` | `intentional-security-constraint` |  | Ambient throw/try/catch is untracked non-local control flow: it exits             scopes the inferred effect row never mentions and skips unwind             obligations (dataspace facet retraction has no checked unwind yet).             The ban is on the ambient form. Since 2026-09-02 the typed abort             ability (slice 1) admits the heads by elaboration: the effect             appears in the inferred row as :abort and the function lowers to             [:result T E], so the ambient form never exists post-elaboration.             Where the unwind precondition would matter, the abort stays refused. |
