...
| `no-implicit-consistency-promotion` | `intentional-semantic-simplification` |  | Consistency is a property of an effect, not of a host or network path. VM selection (ADR-2809051130) and this classification are orthogonal; a surface-status :disposition never sets or widens consistency. |
