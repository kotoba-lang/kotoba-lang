...
| T1.3 | Implement/extend runner: each case runs on **KIR + wasm32-kotoba-v1** minimum — **pilot expanded** (compiler#412–#432 / ADR 0161–0182; **41** pure-product dual-green incl. inc/dec + vector-sum loop; full matrix progressive) | **compiler**, **kotoba-kir**, **kotoba-wasm** | `clojure -M:conformance` green on both | T1.2 | L |
...
| T4.5 | Collections: document O-costs; add only **bounded** map/filter/reduce over hetero-vector/typed-map — **docs landed** + **vector-i64 dual-backend** (compiler#429–#432: mut ops + loop-sum + inc/dec sugar); map/filter/reduce sugar still admission-gated | **compiler**, **kotoba-lang** | No silent O(n²) without docs | T1.1 | M |
