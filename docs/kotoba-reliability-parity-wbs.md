...
| T1.3 | Implement/extend runner: each case runs on **KIR + wasm32-kotoba-v1** minimum — **pilot expanded** (compiler#412–#431 / ADR 0161–0181; **38** pure-product dual-green incl. pred/when-ext/option-string/shift; full matrix progressive) | **compiler**, **kotoba-kir**, **kotoba-wasm** | `clojure -M:conformance` green on both | T1.2 | L |
...
| T4.5 | Collections: document O-costs; add only **bounded** map/filter/reduce over hetero-vector/typed-map — **docs landed** + **vector-i64 dual-backend** (compiler#429+#430: count/at/conj/assoc/drop/get); map/filter/reduce still dual-backend gated | **compiler**, **kotoba-lang** | No silent O(n²) without docs | T1.1 | M |
