...
| T1.3 | Implement/extend runner: each case runs on **KIR + wasm32-kotoba-v1** minimum — **pilot expanded** (compiler#412–#433 / ADR 0161–0183; **42** pure-product dual-green incl. reduce-vector; full matrix progressive) | **compiler**, **kotoba-kir**, **kotoba-wasm** | `clojure -M:conformance` green on both | T1.2 | L |
...
| T4.5 | Collections: document O-costs; add only **bounded** map/filter/reduce over hetero-vector/typed-map — **docs landed** + **vector-i64 dual-backend** + **reduce over vector-i64** (compiler#433 / ADR 0183); map/filter sugar still admission-gated | **compiler**, **kotoba-lang** | No silent O(n²) without docs | T1.1 | M |
