...
| T1.2 | Expand `lang/conformance/manifest.edn` to list **required** backends per case class | **kotoba-lang** | Machine-readable matrix | T1.1 | S |
| T1.3 | Implement/extend runner: each case runs on **KIR + wasm32-kotoba-v1** minimum | **compiler**, **kotoba-kir**, **kotoba-wasm** | `clojure -M:conformance` green on both | T1.2 | L |
...
| T8.3 | Network + secret **signed** reference providers (leave pure allowlist alone) — **partial**: kit EDN receipts provider#36 / ADR 0154; Wasm receipt API provider#37 / ADR 0155 (fixture only); **production AOT Component still open** | **capability-***, **kotoba-core-contracts**, **tamaki**, **kototama**, **provider** | contract-only → signed | T8.1 | XL |
