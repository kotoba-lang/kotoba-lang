...
2. HTTP egress;
   ...
   **W5 deepen ADR 0120–0142:** :bytes leaf + object/http get-stream ready/pending/
   ...
   `resource bytes-task` packaging (ADR 0135) + multi-step Wasmtime of CM resources
...
**W6 secret host transports (2026-07-28, provider#27 / ADR 0146):** fn-fetch + keychain single-get (`-w` only).
...
Next: product verticals / Delivery residual ops CLI shells; live HMAC·AES host adapters (crypto packaging pure landed #83–#85); network/secret kits + signed kit EDN + signed Wasm **receipt API** landed (provider#34–#37 / ADR 0152–0155) — **production AOT** signed Component still pending; wasm-aot still pending. W6 kbb gaps complete; pure capability allowlist reference-impl complete.
3. HTTP ingress and lifecycle;
   ...
   **W5 deepen (2026-07-28, kotoba-component#73 + compiler / ADR 0132):** object-store
   put+get product vertical dual-export packaging + multi-step put-then-get
   Wasmtime sum 3. **Product vertical packaging first slice landed.**
   **W5 deepen (2026-07-28, kotoba-kir#18 + provider#23 + compiler / ADR 0133):** host
   linear resource table for task/stream on reference + nbb (drop + guest consume
   ...
   **W5 deepen (2026-07-28, kotoba-component#76 + compiler#402 / ADR 0135):** full CM
   ...
   surface).
   **W5 deepen (2026-07-28, kotoba-component#77 + compiler / ADR 0136):** correct
   ...
   **W6 cloud-deploy complete + pages-bulk oracle + bare-git audit (2026-07-28, com-cloudflare#5–#10 + murakumo#58).**
   Next: product verticals / Delivery residual; live HMAC·AES host adapters (crypto packaging pure landed #83–#85); network/secret kits + signed receipt APIs landed (provider#34–#37 / ADR 0152–0155); production AOT Wasm + wasm-aot still pending (W6 kbb gaps complete; pure capability allowlist reference-impl complete including cbor/json).
4. state and storage;
...
6. LLM generate/stream/cancel/tool result;
   ...
   **W5 stream-object dual-runtime first slice (2026-07-27, provider#8/#9 +
   ...
   resource packaging + multi-step Wasmtime of CM resources + guest let/if/case move typing
...
**Allowlist pure reference-implemented set complete.** **murakumo#61–#147** pure+adapter path — **full product-shell KIR catalog (32 cores) + portable .cljc pure hosts dual-source complete** (cljs #122–#135 + pure expands #136–#145 + ops/config inject #137+#146+#147). com-cloudflare#17 + com-cloudflare-compat#5 cljs dual-source landed. **T5.3 packs→records complete** murakumo#193–#206 (seats / eligibility / plan / schedule-assign / credits / reconcile / task / rebalance); pure-product profile lists `:record` + `:record-ops`; **no base-N packing remains** in murakumo pure-planner oracles. Next: bool-typed predicate surface (measured **breaking / opt-in** — ADR-reliability-record-access-and-bool-comparisons; wasm32 cannot export `:bool`); T8.3 production AOT for **ops/network** (receipt APIs + package-manifest #36–#40 + **real non-fixture wasm pilot** provider#41–#42 / ADR 0159–0160; readiness signed-wasm still pending for production claim); ops wasm-aot still pending.
...
**T1.3 pilot expand (2026-07-28/29):** compiler#415 / ADR 0164 — pure-product dual-backend pilot **12/12** (full string-ops surface on KIR + wasm32-kotoba-v1); collection fixtures still progressive.
