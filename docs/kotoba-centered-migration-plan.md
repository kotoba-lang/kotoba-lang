...
**Allowlist pure reference-implemented set complete.** **murakumo#61–#139** pure+adapter path — **full product-shell KIR catalog (32 cores) + portable .cljc pure hosts dual-source complete** (cljs #122–#135 + pure expands #136–#139). com-cloudflare#17 + com-cloudflare-compat#5 cljs dual-source landed. Next: residual PVA (schedule eligible? bit-pack intentional); T8.3 production AOT signed Components (receipt APIs landed provider#36–#37 / ADR 0154–0155; readiness signed-wasm still pending); wasm-aot still pending.
