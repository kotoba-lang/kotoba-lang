...
**Allowlist pure reference-implemented set complete.** **murakumo#61–#124** pure+adapter path — **full product-shell KIR catalog (32 cores) + all portable pure hosts dual-source wired** (overlay-driver/runtime #113; Product Value ABI v1 #112–#121; cljs load #122 + dash #123 + token/kekkai #124). Next: Delivery 5–8 remaining shells / residual PVA (schedule eligible? bit-pack intentional) / incremental cljs rewire (secret/connect/report); network/secret caps remain contract-only.
