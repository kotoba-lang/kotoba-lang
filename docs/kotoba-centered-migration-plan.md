...
**Allowlist pure reference-implemented set complete.** **murakumo#61–#147** pure+adapter path — **full product-shell KIR catalog (32 cores) + portable .cljc pure hosts dual-source complete** (cljs #122–#135 + pure expands #136–#145 + ops/config inject #137+#146+#147). com-cloudflare#17 + com-cloudflare-compat#5 cljs dual-source landed. **T5.3 core packs→records landed** murakumo#193–#198 (seats / eligibility / plan); pure-product profile now lists `:record` + `:record-ops`. Next: schedule assign `pack3` residual; bool-typed predicate surface (measured, opt-in — ADR-reliability-record-access-and-bool-comparisons); T8.3 production AOT for **ops/network** (receipt APIs + package-manifest #36–#40 + **real non-fixture wasm pilot** provider#41–#42 / ADR 0159–0160 pure allowlist + grant-binding; readiness signed-wasm still pending for production claim); ops wasm-aot still pending.
