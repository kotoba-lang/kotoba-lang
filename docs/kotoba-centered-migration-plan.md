...
**Allowlist pure reference-implemented set complete.** **murakumo#61–#130** pure+adapter path — **full product-shell KIR catalog (32 cores) + all portable pure hosts dual-source wired** (overlay-driver/runtime #113; Product Value ABI v1 #112–#121; cljs dual-source #122–#130 through overlay+schedule). Next: Delivery 5–8 remaining shells / residual PVA / incremental cljs rewire (infer plan/engine/gc/join/moe/rebalance/relay/credits, overlay runtime/driver, deploy); network/secret caps remain contract-only.
