...
**Allowlist pure reference-implemented set complete.** **murakumo#61–#146** pure+adapter path — **full product-shell KIR catalog (32 cores) + portable .cljc pure hosts dual-source complete** (cljs #122–#135 + pure expands #136–#145 + kekkai config inject #146). com-cloudflare#17 + com-cloudflare-compat#5 cljs dual-source landed. Next: residual PVA (schedule eligible? bit-pack intentional); T8.3 production AOT for **ops/network** (receipt APIs + package-manifest #36–#40 + **real non-fixture wasm pilot** provider#41 / ADR 0159 hash-sha256; readiness signed-wasm still pending for production claim); ops wasm-aot still pending.
