...
- **2026-07-28 murakumo#129:** **cljs dual-source reconcile.plan + component-authority** — `desired`/`deficit`/`action-name`/`watch-sleep-ms` + epoch/sequence/identifier/event-kind when ready. ADR `ADR-260728-w6-cljs-reconcile-cauth-dual`.
- **2026-07-28 murakumo#130:** **cljs dual-source overlay keyring/peer/stream + infer.schedule** — rotation/epoch/preimages; choose-via; window/advance/ack; eligible?/score/queue-inc. ADR `ADR-260728-w6-cljs-overlay-schedule-dual`.
- **2026-07-28 murakumo#131:** **cljs dual-source infer.gc/relay/moe/join** — need/free/target/comfy; make-id/lease/msg kinds; capacity/ratio/verdict; tier/resident/can/relay flags. ADR `ADR-260728-w6-cljs-infer-gc-relay-moe-join`.
- **2026-07-28 murakumo#132:** **cljs dual-source overlay.runtime/driver + deploy.plan** — default ports/adapter-kind/known-adapter?/scheme-host; option-name/blank?/endpoint-kind/dial-ok; constants/manifest-dir/paths/localhost-url/command-output. ADR `ADR-260728-w6-cljs-overlay-deploy-dual`.
...
- **2026-07-28 provider#36–#38:** T8.3 signed **kit EDN** receipts (#36 / ADR 0154) + signed **Wasm provider receipt API** (#37 / ADR 0155, fixture digest only) + **identity.sign inject** adapter (#38 / ADR 0156). Production AOT Component + readiness `:signed-wasm` still pending.

## Next

1. **murakumo product-shell pure dual-source is complete** through **#144** (portable cljs #122–#135, residual `.clj` shells #135, pure expands #136–#144, ops config inject #137). Remaining murakumo work is thin: optional HOME/bin config leave; residual PVA (schedule `eligible?` host bit-pack intentional, arity ≤5); deeper “Still host” folds (envelope maps, placement sorts, SSH subprocess).
2. **Production AOT signed Wasm** for network/secret (kits + signed receipt APIs landed provider#34–#38; readiness `:signed-wasm` still pending).
...
5. Production compiler dependency remains avoided (resource path / precompiled KIR).
