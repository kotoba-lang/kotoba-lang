...
dispatch has portable backend parity.
...
Compiler ADR 0209 / compiler#526 admits an exact record schema forward
declaration and rejects incompatible same-name declarations. Compiler ADR 0210
/ compiler#527 then moves closed-table validation after the bounded same-module
`defrecord` declaration prepass. A recursive `[:ref ...]` may therefore name a
later record without repeating its generated descriptor; exact explicit
declarations remain compatible, undeclared references and incompatible
collisions still fail closed.

Provider#177 / provider ADR 0281 first used exact declarations in twelve
independently built recursive EDN packages, removing the final 36 authored
`record-new` and `record-get` operations while preserving all KIR oracles and
capability wires. Provider#179 / provider ADR 0283 migrates those packages to
the declaration prepass. All twelve explicit descriptors disappear, their
Wasm artifacts remain byte-identical, and only source/provenance identity
changes. This closes the declaration-order aesthetic seam without widening the
schema graph or adding runtime lookup.
