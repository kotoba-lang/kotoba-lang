...
Compiler ADR 0209 / compiler#526 admits a record schema forward declaration
only when it is exactly equal to the descriptor generated by the later
`defrecord`; an incompatible same-name declaration remains a compile error.
Provider#177 / provider ADR 0281 uses that rule in twelve independently built
recursive EDN packages. Their namespace-local `EdnKeyValue` records remove the
final 36 authored `record-new` and `record-get` operations while preserving all
twelve KIR oracles and capability wires. The exact forward declaration keeps
the recursive schema closed and auditable, although its one-time descriptor
