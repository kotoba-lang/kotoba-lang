...
| capability 値渡しの残り（pointer+length/buffer ABI `<op>-with` の compiled threading — i64 shape 以外は interpreter-only・capability contract EDN の kotoba-core-contracts への移管）・signed module（S5 残） | ⬜ | — | — |
...
- **capability 値渡し（S4b、残り）** — 第一スライス（handle-based capability-passing:
  ...
  （i64 shape 以外は interpreter-only）・capability contract EDN の kotoba-core-contracts 移管。
