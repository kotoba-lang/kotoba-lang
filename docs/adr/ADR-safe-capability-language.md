...
| capability 値渡しの残り（CACAO chain 動的照合・typed capability params・全 host-op の compiled threading）・signed module（S5 残） | ⬜ | — | — |
...
- **capability 値渡し（S4b、残り）** — 第一スライス（handle-based capability-passing:
  ...
  effect↔capability 一致 + wasm 実証 shape）は kotoba-lang/kotoba の CLJ runtime slice で達成
  （2026-07-02、docs/lang/capability-values.md「Capability-passing (S4b)」）。残り: CACAO
  delegation chain の動的（署名検証つき）照合・typed capability params（`GraphWriteCap` 型）・
  全 host-op surface の compiled threading。
