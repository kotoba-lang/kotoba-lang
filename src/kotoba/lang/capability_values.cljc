...
(def component-bound-keys
  "Fields required when an ability crosses a Wasm Component provider boundary.
  Basic capability values remain useful for data-level intersection; ordinary
  component execution must use this stricter shape."
  [:cap/target :cap/operation :cap/limits :cap/audit-id])

(defn component-capability?
  "A capability value suitable for a narrow aiueos-owned WIT import. Besides
  normal resource scope it binds a target, one operation, positive byte/item/
  deadline bounds, and a stable audit identity."
  [cap]
  (and (capability? cap)
...
(defn make-component-cap
  "Constructs an ability for component execution. TARGET and OPERATION select
  the narrow WIT/provider surface; LIMITS must contain positive
  :max-bytes/:max-items/:deadline-ms."
...
(defn component-admission
  "Checks the non-resource portion of a component ability against both the
  delegation grant and local policy.  A broad resource grant cannot silently
  authorize a different WIT target, operation, or larger quota.

  Grant shape:  :grant/target, :grant/operations #{op}, :grant/limits.
  Policy shape: :policy/component {kind {:targets #{target}
                                         :operations #{op}
                                         :limits {...}}}.
  All bounds are upper bounds; the request may only narrow them."
