...
(def stale-claim-exceptions
  ...
  [{:heads
   ...
   {:heads #{"kernel-undefined-opcode-handler-address"}
    :as-of "2026-09-07"
    :claimed-by [:guest-grammar :admitted-builtins]
    :reason
    "amu-h7: the address of the canned #UD handler kotoba-native#155 lays in
     the vector-6 entry slot, admitted beside `kernel-double-fault-handler-
     address` in the same wave -- kotoba-gmir (arity 0), kotoba-kir (refusal
     and kernel-native sets), kotoba-sema (`kernel-privileged-operations`),
     kotoba-verifier and kotoba-native (both lowering arms). The kotoba-sema
     this repository's `:test` alias pins predates that arm, so the probe
     measures `operation has no admitted lowering` here while the arm exists
     upstream: this is the `an arm this pin cannot see` case the file
     docstring names, not `no arm anywhere`."
    :closes-when
    "`deps.edn`'s `:test` kotoba-sema pin advances past the merge that adds
     `kernel-undefined-opcode-handler-address 0` to
     `kernel-privileged-operations`. `a-recorded-exception-must-still-be-a-gap`
     goes red naming this head at that advance; delete this group then."}])
