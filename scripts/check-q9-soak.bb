...
(def evidence-path (or (System/getenv "Q9_SOAK_EVIDENCE")
                       "lang/q9-wave1-tranche-1-soak.edn"))
(def evidence (edn/read-string (slurp evidence-path)))
...
(let [{:keys [policy outcome distinct-receipts-per-repository
      ...
      (mapv
       (fn [{:keys [repository dir qualified-revision qualification-artifacts runs]}]
         (let [ids (map :receipt-cid runs)
               ...
               valid-shape?
               (every? (fn [run]
                         (and (re-matches #"[0-9a-f]{64}" (:receipt-cid run ""))
                              (re-matches #"[0-9a-f]{40}" (:root-evidence-sha run ""))
                              (str/starts-with? (:signer run "") "did:key:")
                              (true? (:signer-enrolled run))
                              (true? (:signature-verified run))
                              (re-matches #"[0-9a-f]{40}" (:head-sha run ""))
                              (= policy (:policy run))
                              (= outcome (:outcome run))
                              (str/starts-with? (:gate run "")
                                                (str "test-" (last (str/split repository #"/")) "-"))
                              (try (Instant/parse (:completed-at run)) true
                                   (catch Exception _ false))))
                       runs)
               ready? (boolean
                       (and valid-shape?
                           (= 2 (:kotoba.lang.q9.soak/version evidence))
                           (re-matches #"[0-9a-f]{40}"
                                       (or (get-in evidence [:gate :root-evidence-sha]) ""))
                           (every? #(= (get-in evidence [:gate :root-evidence-sha])
                                       (:root-evidence-sha %)) runs)
                           (re-matches #"[0-9a-f]{40}" (or qualified-revision ""))
                           (= expected-paths (set (keys qualification-artifacts)))
                           (= (count ids) (count (set ids)))
                           (= (count shas) (count (set shas)))
                           (>= (count runs) distinct-receipts-per-repository)
  ...
  (when-not (every? :ready results)
    (fail! "every repository needs distinct successful murakumo receipts over unchanged qualification artifacts and the full soak interval"
