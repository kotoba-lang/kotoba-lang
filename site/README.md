...
## Complete-site recovery and mobile verification

The public landing must retain its architecture, typed-eval, benchmarks,
quickstart, libraries, documentation search, roadmap and community sections.
`site:build` checks those landmarks in every full locale, as well as the blog,
library and legal routes. Do not deploy a hand-written locale stub as a
replacement for the generated site.

On phones the introduction and install action precede the source example;
wide screens retain the code-first layout. Verify 320, 390, 430, 768 and
1440px widths, menu navigation, theme switching and the digest-checked Play
result before publishing. After deployment, compare the live Japanese and
English landing pages and Japanese blog/libraries to the generated bytes;
a successful upload alone does not establish which assets the domain serves.
