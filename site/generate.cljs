...
;; Freebuff conversion measurement is loaded only for visits carrying its
;; bfcid click identifier. Play fetches one same-origin, digest-bound Wasm
;; artifact generated from the checked source.
...
(def theme-toggle theme/control)
...
(def freebuff-bootstrap-js
  "Load Freebuff only when an attributed ad click reaches the page."
...
(def play-js
  (str "document.addEventListener('DOMContentLoaded',function(){"
       ...
       "var clickId=new URLSearchParams(location.search).get('bfcid');if(window.freebuff&&clickId){window.freebuff('conversion','trial_started',{eventId:clickId+':trial_started'});}"
...
(defn footer
  ([] (footer :en "legal/" true))
  ([locale] (footer locale (if (= locale :ja) "../legal/" "legal/") false))
  ([locale legal-href] (footer locale legal-href false))
  ([locale legal-href paid-measurement?]
   (let [ja? (= locale :ja)
         operator (if ja?
     ...
     [:footer {:class "kot-footer"}
      (dds/container
       ...
       [:p {:class "kot-caption kot-muted"}
        ...
        (str/join ", " authority-files) ". "
         (if paid-measurement?
           "Paid campaign conversions may be measured for attribution."
           "No telemetry. No third-party runtime dependency.")]
       [:p
